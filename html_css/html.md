...
* **type** - Определяет тип содержимого: `text/javascript`, `text/vbscript`
* **nomodule** - script should not be executed in browsers that support ES2015 modules (for old browsers)


## `<style>`
### Атрибуты
* **media** - Определяет устройство вывода, для работы с которым предназначена таблица стилей. (`all | print | screen | speech`)
* **type** - Сообщает браузеру, какой синтаксис использовать, чтобы правильно интерпретировать стили: `text/css`
* **scoped** - Если указан этот атрибут, то стиль применяется только внутри своего родительского элемента.
